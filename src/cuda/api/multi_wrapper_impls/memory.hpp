...
inline pool::access_permissions_t access_permissions(const cuda::device_t& device, const pool_t& pool)
{
	return cuda::memory::detail_::access_permissions(device.id(), pool.handle());
}

inline void set_access_permissions(const cuda::device_t& device, const pool_t& pool, pool::access_permissions_t permissions)
{
	cuda::memory::detail_::set_access_permissions(device.id(), pool.handle(), permissions);
}

template <typename DeviceRange>
void set_access_permissions(DeviceRange devices, const pool_t& pool, pool::access_permissions_t permissions)
{
